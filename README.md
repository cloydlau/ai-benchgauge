...
For 千问 Token Plan, the app prefers the official subscription snapshot from the 千问 AI 平台 CLI. Install \`@qianwenai/qianwen-cli\`, run \`qianwen auth login\` with the same account used for the subscription, and confirm \`qianwen usage summary --format json\` reports \`token_plan.subscribed: true\` and the remaining Credits. If the CLI does not return a subscription, the strip shows CC Switch's local request totals instead, labeled “本地”; these totals are not the subscription's remaining Credits.
